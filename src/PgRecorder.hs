...
dispatchNotificationToDb dispatcher pool channel notification = void $ withAsync executeNotification wait
  where
    executeNotification = either (panic "error calling dispatcher") id <$> callProcedure pool (toPgIdentifier dispatcher) channel notification
